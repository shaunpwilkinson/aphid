...
derivePHMM <- function(x, residues = 'auto', gapchar = "-",
                   method = 'background', qe = NULL, qa = NULL, threshold = 0.5,
                   DI = FALSE){
  ...
  if(is.null(qa)){
    if(DI){
      if(any(transtotals == 0)) transtotals <- transtotals + 1
    } else{
      if(any(transtotals[-c(3, 7)] == 0)) transtotals[-c(3, 7)] <- transtotals[-c(3, 7)] + 1
    }
    ...
    if(!DI & (qa[3, 1] != 0 | qa[1, 3] != 0)) stop("DI = FALSE but delete-insert transitions
                                                   enabled in qa matrix. Change DI to TRUE or
                                                   change qa[3, 1] and qa[1, 3] to zero")
  ...
  tmptcs <- tcs ######
  tmptcs[1:3,] <- t(t(tcs[1:3,])/apply(tcs[1:3,], 2, sum))
  tmptcs[4:6,] <- t(t(tcs[4:6,])/apply(tcs[4:6,], 2, sum))
  tmptcs[7:9,] <- t(t(tcs[7:9,])/apply(tcs[7:9,], 2, sum))
  tmptcs[1:3, 1] <- rep(0, 3) # gets rid of NaNs caused by dividing by zero
  ...
  # fromD <- t(t(tcs[1:3,])/apply(tcs[1:3,], 2, sum))
  # fromM <- tcs[4:6,]/apply(tcs[1:3,], 2, sum)
  # fromI <- tcs[7:9,]/apply(tcs[1:3,], 2, sum)
  # tot <- fromD + fromM + fromI
