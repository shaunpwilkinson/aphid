...
#' and file import/export compatible with the HMMER3 software package.
#'
#' @details
...
#'   compatibility with Sean Eddy's \href{http://www.hmmer.org}{HMMER} software suite
#'   through the file input and output functions \code{\link{readPHMM}} and
#'   \code{\link{writePHMM}}.
#'   Those interested are further encouraged to check out Anders Krogh's
#'   \href{https://compbio.soe.ucsc.edu/sam.html}{SAM}
#'   software package, which also features a comprehensive suite of functions and tutorials.
#'
#'   The \pkg{aphid} package is designed to work in conjunction with the "DNAbin"
#'   and "AAbin" object types produced by Emmanuel Paradis's
#'   \code{\link[ape]{ape}} package. This is an essential piece of software for those
...
#'   are written in C++ with the help of Dirk Eddelbuettel's \code{\link[Rcpp]{Rcpp}}
...
#' \pkg{aphid} package
#' are compatible with "DNAbin" and "AAbin" objects generated by the
#' \code{\link[ape]{ape}} package. These object types, in which sequences
#' are represented in a bit-level coding scheme,
#' are recommended for maximizing memory and speed effficiency.
#'
#'
#'
#' @section Functions:
